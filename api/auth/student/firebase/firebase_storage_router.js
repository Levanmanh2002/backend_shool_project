...
router.post('/upload-image', upload.single('file'), async (req, res) => {
    try {
        ...
        const fileDestination = `graduation-certificates/student/${studentId}/${fileName}`;
        ...
        //     $push: { graduationCertificate: fileDestination },
        // });

        res.status(201).json({ message: 'Tải ảnh lên Firebase thành công.' });
    } catch (error) {
...
router.get('/images/:studentId', async (req, res) => {
    try {
        ...
        const folderPath = `graduation-certificates/student/${studentId}/`;
...
router.delete('/delete-images/:studentId', async (req, res) => {
    try {
        ...
        const studentFolder = `graduation-certificates/student/${studentId}`;
        ...
        res.status(200).json({ message: `Đã xóa tất cả hình ảnh trong thư mục của studentId ${studentId}.` });
    } catch (error) {
