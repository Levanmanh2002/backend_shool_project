...
router.get('/all-students', async (req, res) => {
    try {
        const allStudent = await Student.find().populate('major').populate('feesToPay');
...
router.get('/active-students', async (req, res) => {
    try {
        const activeStudents = await Student.find({ status: 1 }).populate('major').populate('feesToPay');
...
router.get('/self-suspended-students', async (req, res) => {
    try {
        const selfSuspendedStudents = await Student.find({ status: 2 }).populate('major').populate('feesToPay');
...
router.get('/suspended-students', async (req, res) => {
    try {
        const suspendedStudents = await Student.find({ status: 3 }).populate('major').populate('feesToPay');
...
router.get('/expelled-students', async (req, res) => {
    try {
        const expelledStudents = await Student.find({ status: 4 }).populate('major').populate('feesToPay');
